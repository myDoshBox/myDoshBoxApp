...
const FAQs = () => {
  ...
  return (
    <>
      ...
      <div class="accordion accordion-flush mt-0 px-0" id="accordionFlushExample">
        
        <div class={`accordion-item border  ${activeAccordion === accordion.id ? "" : "border-top-0"}`}>
          <h2 class={`accordion-header ${activeAccordion === accordion.id ? "accordion-head" : ""}`} id={`flush-heading${accordion.id}`}>
                  <button
                    class={`accordion-button accordion-title ${activeAccordion === accordion.id ? "" : "collapsed"}`}
                    type="button"
                    onClick={() => handleAccordionClick(accordion.id)}
                    aria-expanded={activeAccordion === accordion.id ? "true" : "false"}
          ...
          <div
            ...
            class={`accordion-collapse collapse ${activeAccordion === accordion.id ? "show" : ""}`}
